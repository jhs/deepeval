...
For those looking for a full-blown LLM red teaming orchestration framework, check out [DeepTeam](https://www.trydeepteam.com/). DeepTeam is DeepEval but for red teaming LLMs specifically.
...
If you're looking for decision-tree based LLM-as-a-Judge, check out the [Deep Acyclic Graph (DAG)](/docs/metrics-dag) metric.
